...
def getIonicPotential (r, Z_sph, W, USE_C60, USE_MULLER, targetID):
	...
	if (USE_MULLER == 0 and USE_C60 == 0):
		...
		v0 = getMullerPotential(r, targetID) - 1j * W
...
def getMullerPotential(r, targetID):
	...
	if targetID == 3:
		...
		v = - (1.0 - a1 * np.exp(- a2 * r) + a3 * r * np.exp(- a4 * r) + a5 * r * np.exp(- a6 * r) + a7 * r * np.exp(- a8 * r) + a9 * r * np.exp(- a10 * r)) / r

	return v

def getGroundState (r, x, T, D1, W, P, drdx, ETA_MIX, targetID, n, l, m, occ, N_kso, N_l, KSO_MIN, FROM_SCRATCH, METHOD, USE_MULLER, USE_C60, N_scf=65536, SCF_CONVERGENCE=1E-6, FIGS=1):
	"""
	...
	if FROM_SCRATCH == 1:
		...
		v0 = getIonicPotential (r, Z_sph, W, USE_C60, USE_MULLER, targetID)
	...
	if FIGS == 1:
		...
		plib.linearPlot(r, dft.getLB94 (density * P / np.sqrt(drdx), r, D1, drdx, P, N_g), path="./vxc.png", xmin=0.0, xmax=50.0, ymin=-2.0, ymax=2.0)
