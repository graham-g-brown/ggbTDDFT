...
def propagatorsL (m_active, N_l, LDX):
	...
	for mdx in range(np.max(m_active) + 1):
		...
		temp_tf.tofile("./workingData/hamiltonian/propagatorsL/" + str(LDX).zfill(3) + "/V" + str(mdx).zfill(3) + ".bin")
		temp_tf = LD[mdx, :].astype(np.float64)
		temp_tf.tofile("./workingData/hamiltonian/propagatorsL/" + str(LDX).zfill(3) + "/D" + str(mdx).zfill(3) + ".bin")
...
def propagatorsR (r, T, v0, vscf, maskR, l_active, N_r, N_l, dt, EXP_METHOD):
...
def dfdx (f, dx=1):
	...
	for k in range(1, np.size(f) - 1):
		df[k] = 0.5 * (f[k + 1] - f[k - 1]) / dx
	df[-1] = df[-2]
	df[0] = df[1]
	return df
...
def getMaxR (R_MASK, EMIN):

	kmin = np.sqrt(2.0 * EMIN)
	c 	 = 2.62206
	delta = 0.20

	R_MAX = R_MASK + c / kmin / delta / 2.0

	return R_MAX

def getABCR (r, R_MASK, R_MAX, EMIN=0.01):
	...
	x = 2.0 * delta * kmin * (r[minDX : np.size(r)] - R_MASK)
...
def generateGaussLobattoGrid(N_r, R_max, zeta):
...
def getSphericalElectronConfiguration (targetID):
	...
	if (targetID == 0):
		...
		N_kso    = np.size(n)
		NUM_L 	 = np.size(np.unique(l))
		active   = np.array([0, 0, 0, 1, 0, 0, 1, 1])
...
def getSimulationStates (targetID):
	...
	if targetID == 0:
		...
		n 	     = np.array([1, 2, 3, 4, 2, 3, 4, 3 ])
		l        = np.array([0, 0, 0, 0, 1, 1, 1, 2 ])
		sdx = np.array([2, 3, 5, 5, 6, 6, 7, 7, 7])
